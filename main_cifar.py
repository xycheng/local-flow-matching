...
from datasets_images import make_cifar10_loaders, infinite_loader
...
def random_hflip_batch(x: torch.Tensor, p: float = 0.5) -> torch.Tensor:
    """
    Random horizontal flip for a batch tensor x in NCHW.
    Works whether x is from torchvision CIFAR or your pushed TensorDataset.
    Assumes x is float tensor (on CPU or GPU is fine).
    """
    if p <= 0.0:
        return x
    if p >= 1.0:
    ...
    if mask.any():
        x = x.clone()  # avoid in-place on views / underlying storage
        x[mask] = x[mask].flip(-1)  # flip W dimension
    return x
...
class CNF(nn.Module):
    def __init__(self, in_shape,  dim,  dim_mults, 
        ...
        self.net = UNetModelWrapper(
                        ...
                        num_head_channels= num_head_channels, #64, #32
...
def run_final_eval(*, master_dir: str, seed: int, L: int, in_shape, 
                   hidden_dim: int, dim_mults, num_res_blocks: int, 
                   num_head_channels: int, dropout: float, ode_cfg: dict,
...
if __name__ == '__main__':
    ...
    train_dl, test_dl = make_cifar10_loaders(
        root=root,
        batch_size=batch_size,
        num_workers=num_workers,
        pin_memory=pin_memory,
    ...
    for l in range(1, L+2):
        ...
        for batch in tqdm(range(start_batch, max_batch), ncols=88):
            ...
            if args_yaml['training']['flip_aug']:
                x0 = random_hflip_batch(x0, p=0.5)
            ...
            if l == L+1:
                ...
                if coupling == "dependent":
                    ...
                    if args_yaml['training']['flip_aug']:
                        x0p = random_hflip_batch(x0p, p=0.5)
                else:
