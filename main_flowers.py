...
from datasets_images import make_flowers_loaders, infinite_loader
...
class CNF(nn.Module):
    def __init__(self, in_shape,  dim,  dim_mults, 
                        num_res_blocks, num_head_channels, dropout, ode_cfg):
        super().__init__()
        self.net = UNetModelWrapper(
                        ...
                        num_heads= 4, # will be override
                        num_head_channels= num_head_channels, #32
                        attention_resolutions="16",
                        dropout=dropout) #0 or .1
...
@torch.no_grad()
def push_loader_by_current(flow, dl, *, device):
    """
    Push all samples in dl through the flow block
    returning a single CPU tensor pool (N,C,H,W).
    """
    flow.eval()
    outs = []
    for batch in tqdm(dl, desc="Pushforward all", ncols=88):
        ...
        y = traj[-1].detach().cpu()                      # back to CPU
        outs.append(y)
    return torch.cat(outs, dim=0)  # CPU tensor
...
def make_fid_gen(lfm, in_shape, device):
    """
    CleanFID-compatible generator:
      gen(z) -> torch.uint8 images in (B,3,H,W), range [0,255]
    Assumes lfm.sample returns float in [-1,1] OR [0,1] (we handle both).
    """
    C, H, W = in_shape
    @torch.no_grad()
    def gen(z):
        ...
        img = lfm.sample(z_img, return_list=False)  # (B,3,H,W) float in [0,1]
...
def run_final_eval(*, root: str, master_dir: str, seed: int, L: int,
                   in_shape, hidden_dim: int, dim_mults, num_res_blocks: int, num_head_channels: int,
                   dropout: float, ode_cfg: dict,
    ...
    for l_now in range(1, L + 2):  # blocks are 1..L+1
        f = CNF(in_shape, hidden_dim, dim_mults, num_res_blocks, num_head_channels, dropout, ode_cfg).to(device)
    ...
    if cal_fid:
        ...
        from datasets_images import _tfm_to_minus1_1, dump_real_png_subset  # datasets_images.py
        C, H, W = in_shape
        assert C == 3, "Flowers102 is RGB; expected in_shape[0]==3"
        gen = make_fid_gen(lfm, in_shape=in_shape, device=device)
    
        stats_name = f"flowers102_{H}_all_cached_stats"
        real_dir = os.path.join(root, f"flowers102_ref_png_{H}")
    
        tfm_real = _tfm_to_minus1_1(image_size=H)
        flowers_root = os.path.join(root, "flowers")
    
        ds_train = tv_datasets.Flowers102(root=flowers_root, split="train", download=True, transform=tfm_real)
        ds_val   = tv_datasets.Flowers102(root=flowers_root, split="val",   download=True, transform=tfm_real)
        ds_test  = tv_datasets.Flowers102(root=flowers_root, split="test",  download=True, transform=tfm_real)
    
        real_ds = ConcatDataset([ds_train, ds_val, ds_test])
        print(f"[Flowers102 FID] real_ds size = {len(real_ds)}")  # should print 8189
    
        if not fid.test_stats_exists(stats_name, mode="legacy_tensorflow"):
            if not os.path.isdir(real_dir) or len(os.listdir(real_dir)) == 0:
                dump_real_png_subset(real_ds, real_dir, n=len(real_ds))
...
if __name__ == '__main__':
    ...
    root = args_yaml['data'].get('root', './data')
    ...
    num_head_channels = int(args_yaml['model']['num_head_channels']) 
    print(f"Unet: base channels: {hidden_dim}, dim_mults: {dim_mults},\
            res depth: {num_res_blocks}, head channels: {num_head_channels}")
    ...
    for l in range(1, L+2):
        ...
        flow = CNF(in_shape, hidden_dim,  dim_mults, num_res_blocks, num_head_channels, dropout, ode_cfg).to(device)
        print(f'Number of parameters: {count_params(flow):.3f}k')
        ema = deepcopy(flow).to(device) #keep a copy for ema
        ema.eval()  
        
        #if l == 1, just use the initial train_dl, and flow from scratch
        if l > 1:
            if len(flow_ls) == l - 1: #check if the length of flow_ls is correct
                # warm start if asked so
                ...
                raise RuntimeError(
                    f"[block {l}] flow_ls length mismatch: "
        ...
        if resume and resume_path is not None:
            sdict = torch.load(resume_path, map_location=device, weights_only= False)
            ...
            if 'ema_state_dict' in sdict:
                ema.load_state_dict(sdict['ema_state_dict'])
            else:
        ...
        #------ batch loops, restart when exhausted ------
